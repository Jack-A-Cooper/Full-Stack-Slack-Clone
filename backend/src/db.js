...
getFullName = async (ident) => {  // A helper func for posting. Retrieves the full name of the current user's id to use in 'from' part
  ...
  const { rows } = await pool.query(query);
  console.log('inside getFullName. About to print rows')
  console.log(rows);
  console.log('about to print firstname');
  console.log(rows[0].firstname);
  console.log('about to print firstname with toString');
  console.log(rows[0].firstname.toString());
